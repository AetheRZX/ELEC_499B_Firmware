...
----------------------------------------------------------------------------*/
#define DBCTL_INIT_STATE  BP_DISABLE /*(BP_ENABLE + POLSEL_ACTIVE_HI_CMP)*/
...
typedef struct {	
		_iq		Duty1;			// input:  Duty cycle input range [0, 1.0]
		_iq     Duty2;          // input:  Duty cycle input range [0, 1.0]
		_iq     Duty3;          // input:  Duty cycle input range [0, 1.0]
		_iq     Duty4;          // input:  Duty cycle input range [0, 1.0]
		_iq     Duty5;          // input:  Duty cycle input range [0, 1.0]
		_iq     Duty6;          // input:  Duty cycle input range [0, 1.0]
...
------------------------------------------------------------------------------*/
#define F2803X_PWM_CNTL			{_IQ(0.0), \
                                 _IQ(0.0), \
                                 _IQ(0.0), \
                                 _IQ(0.0), \
                                 _IQ(0.0), \
...
#define PWM_CNTL_INIT_MACRO(v)														\
		...
		EPwm1Regs.CMPB=v.PWMprd/2;                                                  \
		EPwm2Regs.CMPA.half.CMPA = v.PWMprd/2;										\
		EPwm2Regs.CMPB=v.PWMprd/2;                                                  \
		EPwm3Regs.CMPA.half.CMPA = v.PWMprd/2;										\
		EPwm3Regs.CMPB=v.PWMprd/2;                                                  \
																					\
		...
		EPwm1Regs.AQCTLB.all = PWM_CNTL_AQCTLB_INIT_STATE;                          \
		EPwm2Regs.AQCTLA.all = PWM_CNTL_AQCTLA_INIT_STATE;                         	\
		EPwm2Regs.AQCTLB.all = PWM_CNTL_AQCTLB_INIT_STATE;                          \
		EPwm3Regs.AQCTLA.all = PWM_CNTL_AQCTLA_INIT_STATE;                         	\
		EPwm3Regs.AQCTLB.all = PWM_CNTL_AQCTLB_INIT_STATE;                          \
																					\
		...
		EPwm3Regs.PCCTL.all = PWM_CNTL_PCCTL_INIT_STATE;                           	\
 		...
 		EPwm1Regs.TZSEL.all = PWM_CNTL_TZSEL_INIT_STATE;                            \
 		EPwm2Regs.TZSEL.all = PWM_CNTL_TZSEL_INIT_STATE;                            \
 		EPwm3Regs.TZSEL.all = PWM_CNTL_TZSEL_INIT_STATE;                            \
 		                                                                            \
...
#define PWM_CNTL_MACRO(v)                                                              /*                                                  */\
    uiDuty1 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty1))) >> 18);      /*                                                  */\
    uiDuty2 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty2))) >> 18);      /*                                                  */\
    uiDuty3 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty3))) >> 18);      /*                                                  */\
    uiDuty4 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty4))) >> 18);      /*                                                  */\
    uiDuty5 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty5))) >> 18);      /*                                                  */\
    uiDuty6 = (Uint16) ((_IQ18mpy((_iq)v.PWMprd<<18, _IQtoIQ18(v.Duty6))) >> 18);      /*                                                  */\
                                                                                       /*                                                  */\
    EPwm1Regs.CMPA.half.CMPA = (uiDuty1);                                              /*                                                  */\
    EPwm1Regs.CMPB = (uiDuty2);                                                        /*                                                  */\
    EPwm2Regs.CMPA.half.CMPA = (uiDuty3);                                              /*                                                  */\
    EPwm2Regs.CMPB = (uiDuty4);                                                        /*                                                  */\
    EPwm3Regs.CMPA.half.CMPA = (uiDuty5);                                              /*                                                  */\
    EPwm3Regs.CMPB = (uiDuty6);                                                        /*                                                  */\
                                                                                       /*                                                  */\
